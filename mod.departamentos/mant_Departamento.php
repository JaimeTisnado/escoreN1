...
$Submit				= $_POST['Submit'];
$mantNombre			= "Agregar";
define('pUrl', "listado.php?id=".$IDPais);
$sArrayPais			= fndb_getPaisbyId($IDPais);

if(isset($ID)){
$mantNombre	= "Editar";
fndb_getDepartamentobyId($ID);
}
...
if ( strlen(trim($txtCodigo)) == 0 ){
	...
	if($s_eNombre != 1 && $s_eCodigo != 1){
		...
		if(isset($ID)){
			...
			$nExiste	= fndb_existeDepartamento($txtCodigo);
			if ($nExiste == 0 || $txtCodigo == $sArray[strtolower('codigoDepartamento')]){
				fndb_editarDepartamento($idUsuario, $IDPais, $ID, $txtNombre, $txtCodigo, $rbPub);
				$cssMensaje	= fn_getCssMensaje(1);
				$lblMensaje	= "Departamento Modificado Correctamente";
				echo '<script type="text/javascript">
				window.history.go(-2);
				</script>';
			}else{
				$cssMensaje	= fn_getCssMensaje(2);
				$lblMensaje	= "El Código del Departamento ya se encuentra registrado";
			}
		}
		else {
			...
			$nExiste	= fndb_existeDepartamento($txtCodigo);
			if ($nExiste == 0){
				fndb_nuevoDepartamento($idUsuario, $IDPais, $txtNombre, $txtCodigo, $rbPub);
...
$txtNombre 			= ( (isset($txtNombre)) ? $txtNombre : $sArray[strtolower('nomDepartamento')] ); 
$txtCodigo 			= ( (isset($txtCodigo)) ? $txtCodigo : $sArray[strtolower('codigoDepartamento')] ); 
...
<div class="column100">
    ...
    <div class="widget">
    	<div class="widget_header">
            <i class="icon-widget fa-list"></i><h3><?php echo $mantNombre ?> Departamento [<?php echo $sArrayPais[strtolower('nomPais')]; ?>]</h3>
        </div>
            	...
            	<form id="form1" name="form1" method="post" action="">
                      ...
                      <label>No</label>
                          ...
                          <tr>
                            ...
                            <a href="javascript:;" class="btn" onclick="jsRegresar('listado.php?id=<?php echo $IDPais; ?>')">Cancelar</a>
                            </td>
                          </tr>
                          <tr>
                            <td>&nbsp;</td>
