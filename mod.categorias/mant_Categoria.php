...
?>
<?php
define('pUrl', JPATH_BASE_WEB."/mod.categorias/listado.php");
...
if ( strlen(trim($txtCodigo)) == 0 ){
	...
	if($s_eNombre != 1 && $s_eCodigo != 1){
		...
		if(isset($ID)){
			$nExiste	= fndb_existeCategoria($txtCodigo);
			if ($nExiste == 0 || $txtCodigo == $sArray[strtolower('codigoCategoria')]){
				fndb_editarCategoria($idUsuario, $ID, $txtNombre, $txtCodigo, $txtMemo, $rbPub);
				$cssMensaje	= fn_getCssMensaje(1);
				$lblMensaje	= "Categoria Modificada Correctamente";
				...
				$lblMensaje	= "El Código de Categoria ya se encuentra registrado";
			}
		}
		else {
			$nExiste	= fndb_existeCategoria($txtCodigo);
			if ($nExiste == 0){
				fndb_nuevaCategoria($idUsuario, $txtNombre, $txtCodigo, $txtMemo, $rbPub);
				header("Location: ".pUrl."");
			}else{
				$cssMensaje	= fn_getCssMensaje(2);
				$lblMensaje	= "El Código de Categoria ya se encuentra registrado";
...
<div class="column100">
    ...
    <div class="widget">
    	<div class="widget_header">
            <i class="icon-widget fa-list"></i><h3><?php echo $mantNombre ?> Categoria</h3>
        </div>
