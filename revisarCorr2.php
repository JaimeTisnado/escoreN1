...
$sqlDoble = "SELECT * FROM inscripciones WHERE nie='$nie' AND anioLectivo='$anioLectivo';";
$resultadoDoble = mysqli_query($conexion, $sqlDoble);
if ($resultadoDoble && mysqli_num_rows($resultadoDoble) > 0) {
    $sArrayDoble = mysqli_fetch_array($resultadoDoble);
    if (is_array($sArrayDoble) && isset($sArrayDoble['anioLectivo'])) {
        $anioLectivoDoble = $sArrayDoble['anioLectivo'];
    } else {
        $anioLectivoDoble = "Valor no encontrado";
    }
} else {
    $anioLectivoDoble = "Consulta inválida o sin datos";
}
