...
if ( strlen(trim($txtCodigo)) == 0 ){
	...
	if($s_eNombre != 1 && $s_eCodigo != 1){
		...
		if(isset($ID)){
			...
			$nExiste	= fndb_existeGrado($txtCodigo);
			if ($nExiste == 0 || $txtCodigo == $sArray[strtolower('codigoGrado')]){
				fndb_editarGrado($idUsuario, $ID, $txtNombre, $txtCodigo, $txtMemo, $rbPub);
				$cssMensaje	= fn_getCssMensaje(1);
				$lblMensaje	= "Grado Modificado Correctamente";
				echo '<script type="text/javascript">
				window.history.go(-2);
				</script>';
			}else{
				$cssMensaje	= fn_getCssMensaje(2);
				$lblMensaje	= "El Código del Grado ya se encuentra registrado";
			}
		}
		else {
			$nExiste	= fndb_existeGrado($txtCodigo);
			if ($nExiste == 0){
				fndb_nuevoGrado($idUsuario, $txtNombre, $txtCodigo, $txtMemo, $rbPub);
...
$txtNombre 			= ( (isset($txtNombre)) ? $txtNombre : $sArray[strtolower('nomGrado')] ); 
$txtCodigo 			= ( (isset($txtCodigo)) ? $txtCodigo : $sArray[strtolower('codigoGrado')] ); 
$txtMemo 			= ( (isset($txtMemo)) ? $txtMemo : $sArray[strtolower('memoGrado')] ); 
...
<div class="column100">
    ...
    <div class="widget">
    	<div class="widget_header">
            <i class="icon-widget fa-list"></i><h3><?php echo $mantNombre ?> Grado</h3>
        </div>
            	...
            	<form id="form1" name="form1" method="post" action="">
                        <table cellpadding="0" cellspacing="1">
                          ...
                          <tr>
                            ...
                            <textarea name="txtMemo" id="txtMemo" cols="30" rows="4"><?php echo $txtMemo; ?></textarea>                             </td>
                          </tr>
                          <tr>
                            <td>&nbsp;</td>
                            <td>&nbsp;</td>
