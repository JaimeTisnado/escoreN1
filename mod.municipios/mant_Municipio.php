...
$Submit				= $_POST['Submit'];
$mantNombre			= "Agregar";
define('pUrl', "listado.php?id=".$IDDepto);
$sArrayDepto			= fndb_getDepartamentobyId($IDDepto);

if(isset($ID)){
$mantNombre	= "Editar";
fndb_getMunicipiobyId($ID);
}
...
if ( strlen(trim($txtCodigo)) == 0 ){
	...
	if($s_eNombre != 1 && $s_eCodigo != 1){
		...
		if(isset($ID)){
			...
			$sRowDepto	= fndb_getDepartamentobyId($IDDepto);
			$nExiste	= fndb_existeMunicipio($sRowDepto[strtolower('codigoDepartamento')],$txtCodigo);
			if ($nExiste == 0 || $txtCodigo == $sArray[strtolower('codigoMunicipio')]){
				fndb_editarMunicipio($idUsuario, $IDDepto, $ID, $txtNombre, $txtCodigo, $rbPub);
				$cssMensaje	= fn_getCssMensaje(1);
				$lblMensaje	= "Municipio Modificado Correctamente";
				echo '<script type="text/javascript">
				window.history.go(-2);
				</script>';
			}else{
				$cssMensaje	= fn_getCssMensaje(2);
				$lblMensaje	= "El Código del Municipio ya se encuentra registrado";
			}
		}
		else {
			$sRowDepto	= fndb_getDepartamentobyId($IDDepto);
			$nExiste	= fndb_existeMunicipio($sRowDepto[strtolower('codigoDepartamento')],$txtCodigo);
			if ($nExiste == 0){
				fndb_nuevoMunicipio($idUsuario, $IDDepto, $txtNombre, $txtCodigo, $rbPub);
...
$txtNombre 			= ( (isset($txtNombre)) ? $txtNombre : $sArray[strtolower('nomMunicipio')] ); 
$txtCodigo 			= ( (isset($txtCodigo)) ? $txtCodigo : $sArray[strtolower('codigoMunicipio')] ); 
...
<div class="column100">
    ...
    <div class="widget">
    	<div class="widget_header">
            <i class="icon-widget fa-list"></i><h3><?php echo $mantNombre ?> Municipio [<?php echo $sArrayDepto[strtolower('nomDepartamento')]; ?>]</h3>
        </div>
            	...
            	<form id="form1" name="form1" method="post" action="">
                        <table cellpadding="0" cellspacing="1">
                          ...
                          <tr>
                            ...
                            <td><select name="cmbDepto" id="cmbDepto" disabled="disabled">
                      ...
                      <label>No</label>
                          ...
                          <tr>
                            ...
                            <a href="javascript:;" class="btn" onclick="jsRegresar('listado.php?id=<?php echo $IDDepto; ?>')">Cancelar</a>
                            </td>
                          </tr>
                          <tr>
                            <td>&nbsp;</td>
