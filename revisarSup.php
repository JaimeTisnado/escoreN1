...
#Revisar si la calificacion es igual a una de los revisores
$vCalificacion = fndb_validaCalificacion($idImagen,$anioLectivo,$rbCalificacion);
if( $vCalificacion == 0 ) { #no se encontro igualdal, revisar adyacencia
	
	$sSql	= fndb_getCalificacionesRevisores($idImagen,$anioLectivo);
	while ($sRow = fn_ExtraerQuery($sSql)){
		
		$calificacion 	= $sRow[strtolower('calificacion')];
		$nAdyacente		= abs($calificacion - $rbCalificacion);
		if ($nAdyacente == 1){ #Cumple Adyacencia, calificar con la calificacion mayor.
			$nArray				= array($calificacion,$rbCalificacion);
	...
	if ($sCalificado == false){#No cumple con los revisores, enviar notificacion al Coordinador.
		$rCalificacion		= fndb_calificarItem($fechaInicio, $idUsuario, $anioLectivo, $idImagen, $rbCalificacion, $rMemoScore);
		fndb_enviarMensajeCoordinadorItem($idUsuario, $idItem, $IDItemImagen, $idImagen);
	}
	
}else{#asignar calificacion del supervisor porque la calificacion es igual a uno de los revisores.
	
	#Validar no este revisado el item para otorgar la calificacion.
	$vRevisado		= fndb_validaRevision($anioLectivo, $idImagen);
	if ($vRevisado == 0){
		$rCalificacion		= fndb_calificarItem($fechaInicio, $idUsuario, $anioLectivo, $idImagen, $rbCalificacion, $rMemoScore);
		$rCalificacionFinal	= fndb_marcarCalificacionFinal($idImagen, $anioLectivo, $rbCalificacion);
	}else{
		$rCalificacion	= 3;
		$rCalificacionDoble	= fndb_calificarItemDoble($fechaInicio, $idUsuario, $anioLectivo, $idImagen, $rbCalificacion, $rMemoScore);
	}#vRevisado
...
#============ OBTENER IMAGEN A CALIFICAR (SUPERVISOR) ==============

normal:
#Imagen a Calificar
fndb_getImagenItemCalificarSupervisor();
$bFlagAnclaje	= true;
